...
static void Prepare_CBR_BitsTable(gx_fast_enc_drv_t* p, bool rc)
{
    ...
    if (block_size == 0) {
        VLOG(DEBUG,"block_width:%d, block_height:%d!!", p->block_width, p->block_height);
        block_size = 1;
    }
...
//    }
    ...
    if (rc) {
        //sam mdf
#ifdef FASTGX_AVC_FIX_QP_CBR

        uint32_t qp_base =p->quant | p->quant << 8 | p->quant << 16 | p->quant << 24;
#else
        uint32_t qp_base =qp | qp << 8 | qp << 16 | qp << 24;
#endif
        //end

        VLOG(NONE, "sam debug qp_tbl >> Fill_CBR_Table_Custom rc 1:qp = 0x%x,quant = 0x%x,qp_base = 0x%x,p->qp_tbl[0] = 0x%x,p->qp_tbl[1] = 0x%x\n",qp,p->quant,qp_base,p->qp_tbl[0],p->qp_tbl[1]);
        //sam mdf
        //
#ifdef FASTGX_AVC_FIX_QP_CBR
        qp_step = 0;
